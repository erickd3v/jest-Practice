...
const countVowelsAndConsonants = (string) => {
  ...
  const vowelArray = Array.from(lowercaseString).filter((char) =>
    vowels.includes(char)
  );
  const consonantArray = Array.from(lowercaseString).filter(
    (char) => !regex.test(char) && !vowels.includes(char)
  );
  return `
